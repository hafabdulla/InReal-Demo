...
2. Check `database/pg/` for the current schema — numbered migration files (`01` through `20`, plus `08b`, which is numbered that way because 09 and 10 ALTER a table 08b creates).
   ...
   **Use the migration runner, not manual pastes.** `npm run db:status` shows what a database has and what it is missing; `npm run db:migrate` applies the pending ones. It tracks state in a `schema_migrations` table, runs each file in its own transaction, and refuses to run if a migration that was already applied has since been edited.
   ...
   The old `npm run db:setup` still exists but only knows files `01` and `02` by name — prefer the runner. **As of 17 Sep the two databases differ on purpose: local is at 20 (21 recorded, 0 pending, counting 08b), production is still at 19 with migration 20 pending.** Migration 20 (`investor_requests` and `investor_request_events`, for express interest) creates two empty tables that no code reads yet, so the drift is harmless — but it goes to production **before** the endpoint that writes to them, like every other one. Migrations 18 (`properties.is_published`) and 19 (`property_valuations`, plus `properties.property_value` made nullable) went to production before the code that reads them: the investor property list reads both, so the reverse order 500s every property page. Migration 16 was the earlier instance of the same trap: it added columns read on the login path (`LOGIN_USER_COLUMNS`), and deploying ahead of it would have 500'd every login on both portals.
