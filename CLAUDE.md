...
2. Check `database/pg/` for the current schema — numbered migration files (`01` through `20`, plus `08b`, which is numbered that way because 09 and 10 ALTER a table 08b creates).
   ...
   **Use the migration runner, not manual pastes.** `npm run db:status` shows what a database has and what it is missing; `npm run db:migrate` applies the pending ones. It tracks state in a `schema_migrations` table, runs each file in its own transaction, and refuses to run if a migration that was already applied has since been edited.
   ...
   The old `npm run db:setup` still exists but only knows files `01` and `02` by name — prefer the runner. **As of 17 Sep, local and production are both at 20 (21 recorded, 0 pending, counting 08b).** Migration 20 (`investor_requests` and `investor_request_events`, for express interest) went to production while the tables were still empty and nothing read them — ahead of the endpoint that will write to them, like every other one. Migrations 18 (`properties.is_published`) and 19 (`property_valuations`, plus `properties.property_value` made nullable) went to production before the code that reads them: the investor property list reads both, so the reverse order 500s every property page. Migration 16 was the earlier instance of the same trap: it added columns read on the login path (`LOGIN_USER_COLUMNS`), and deploying ahead of it would have 500'd every login on both portals.
...
| `MIN_INDICATIVE_AMOUNT` | Express interest | The smallest amount an investor may register interest for, in USD. Defaults to **3000**, decided 17 Sep from the figure in the pilot investor pack. The public site was updated to match the same day — it said "$500" in **ten** places across six components, not the two the tracker had recorded. The refusal names the minimum back to the caller, so **never copy this figure into the frontend as a literal**; if it changes again, change it here first and let the site follow |
...
- **A property's money figures change only through a valuation.** `property_value`, `monthly_rental_income`, `projected_annual_yield` and `fraction_price` on `properties` are the projection of the most recent `property_valuations` row by valuation date, written by `POST /api/ops/properties/:id/valuations` (`FINANCE_ROLES`) in the same transaction as the ledger insert, and by nothing else. The edit endpoint refuses them by name, and the price per fraction is always value ÷ fractions, never typed. Same rule as `portfolio_adjustments`. A draft cannot be published until one exists (D.45).
...
- **The `hidden` attribute does not hide an element whose class sets `display`.** `ops-admin-portal/styles.css` defines `.hidden { display: none !important }` but has no `[hidden]` rule, so the browser's own `[hidden]` styling loses to any class that sets a display — `el.hidden = true` silently does nothing on a `.kyc-detail-item`, a `.media-grid` or a `.ghost-btn`. Use the `.hidden` class for those and keep the attribute for plain containers. Caught while building the property flow; the element looked hidden in the code and was visible on screen.
...
5. **Update the living docs before the session ends.** `InReal_Master_Tracker.md` always — a new or extended entry saying what was built, what was decided and how it was tested, plus the Plain-English Status row so the table never drifts behind the log. `CLAUDE.md` too whenever the change adds a migration, an env var, a rule, or a trap worth not rediscovering. This is the step that has been skipped most often, so it is also enforced: a `Stop` hook (`.claude/hooks/docs-drift.mjs`, wired in `.claude/settings.json`) compares both repos against where they stood at the start of the session, and if code moved while the tracker did not, it says so once. It reminds; it never writes — deciding what an entry should say is the judgement the tracker exists to capture.
...
**Know the shape of its net before trusting it.** The source scan only inspects interpolations whose variable name it recognises — `user`, `intent`, `doc`, `row`, `req`, `r`, `h`, `u`, `detail` (`TAINTED_ROOT`). A server record held in a variable called `p`, `item` or `property` passes the scan untouched, however unescaped it is. So it catches the mistake it was written for and not the general case: name server data `row` or `detail` in new render code, or add your name to that list. It is a tripwire, not a proof.
