...
2. Check `database/pg/` for the current schema — numbered migration files (`01` through `19`, plus `08b`, which is numbered that way because 09 and 10 ALTER a table 08b creates).
   ...
   **Use the migration runner, not manual pastes.** `npm run db:status` shows what a database has and what it is missing; `npm run db:migrate` applies the pending ones. It tracks state in a `schema_migrations` table, runs each file in its own transaction, and refuses to run if a migration that was already applied has since been edited.
   ...
   The old `npm run db:setup` still exists but only knows files `01` and `02` by name — prefer the runner. **As of 15 Sep, local has everything through 19 (20 recorded, counting 08b); production has through 17 (18 recorded).** Migrations 18 (`properties.is_published`) and 19 (`property_valuations`, and `properties.property_value` made nullable) are owed in production, and the investor property list reads both — so they go to production before the push, or every property page 500s. Migration 16 was the earlier instance of the same trap: it added columns read on the login path (`LOGIN_USER_COLUMNS`), and deploying ahead of it would have 500'd every login on both portals.
