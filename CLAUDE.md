...
| `TOTP_ENCRYPTION_KEY` | 2FA + bank details + **operator login** | 64 hex chars (32 bytes). Encrypts TOTP secrets AND bank account numbers — one key for both, deliberately (same risk profile, no reason for two). **Now a hard boot requirement** (since 05 Aug, when 2FA began gating operator login) — the server refuses to start without a well-formed key, because a missing one would lock out every enrolled operator with a 500 from inside a decrypt call rather than a diagnosable message |
