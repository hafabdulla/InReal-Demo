...
| `MIN_INDICATIVE_AMOUNT` | Express interest | The smallest amount an investor may register interest for, in USD. Defaults to **3000**, decided 17 Sep from the figure in the pilot investor pack. ⚠️ **The public homepage still advertises "from $500"** in `src/components/Hero.jsx` (paragraph and tile), so until that copy follows, a prospect can be refused by a number they were never shown. The refusal names the minimum back to the caller — **never copy this figure into the frontend as a literal** |
