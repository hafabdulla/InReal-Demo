...
| `TOTP_ENCRYPTION_KEY` | 2FA + bank details | 64 hex chars (32 bytes). Encrypts TOTP secrets AND bank account numbers — one key for both, deliberately (same risk profile, no reason for two). NOT a hard boot requirement; missing key just 500s those specific endpoints |
